...
直近の run が失敗していれば private repo に追跡 issue を作成し GAS 経由で admin にメール通知。
...
OWNER = "yasumorishima"
PUB_REPO = "minami-public-cron"
PRIV_REPO = "minami-baseball-ob"
APP_NAME = "minami-baseball-ob-bot"
    ...
    for wf, st in statuses.items():
        ...
        if is_bad:
            ...
            body = ("自動監視 (health-check) が member パイプラインのワークフロー失敗を検出しました。\n\n"
                    ...
                    "※ この issue は復旧 (最新 run が success) を検出すると自動 close されます。手動 close も可。") % (wf, PUB_REPO, st["conclusion"], st["created"], st["url"], APP_NAME)
            delivered = False
            issue_url = st["url"]
            try:
